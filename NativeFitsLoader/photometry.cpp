...
namespace Photometry
{
	...
	bool Extractor::Extract(Loader::FITSInfo& fit, std::valarray<float>& image, Catalog** catalog_out, int* status, Callback callback)
	{
		...
		*catalog_out = new Catalog();
		...
		if (*status = sep_background(&simage, m_parameters.background_tile_size, m_parameters.background_tile_size, m_parameters.background_filter_size, m_parameters.background_filter_size, median, &catalog->background))
		{
			if (catalog->background != nullptr)
			{
				sep_bkg_free(catalog->background);
				catalog->background = nullptr;
			}
			return false;
		}

		// Subtract background model from work image
		if (*status = sep_bkg_subarray(catalog->background, work_image_ptr, SEP_TFLOAT))
		{
			if (catalog->background != nullptr)
			{
				sep_bkg_free(catalog->background);
		...
		simage.noiseval = sep_bkg_globalrms(catalog->background);
		...
		if (*status = sep_extract(&simage, m_parameters.extract_threshold, SEP_THRESH_REL, m_parameters.extract_min_obj_pixels, filter, filter_width, filter_height, SEP_FILTER_MATCHED, m_parameters.extract_deblend_nthresh, m_parameters.extract_deblend_contrast, 1, m_parameters.extract_cleaning_aggressiveness, &catalog->catalog))
		{
			sep_bkg_free(catalog->background);
			catalog->background = nullptr;
			if (catalog->catalog != nullptr)
			{
				sep_catalog_free(catalog->catalog);
		...
		for (int i = 0; i < nobj; i++)
		{
			...
			double a = catalog->catalog->a[i];
			double b = catalog->catalog->b[i];
			...
			if (rab > 0.8 && rab < 10)
			{
				...
				double x = catalog->catalog->x[i];
				double y = catalog->catalog->y[i];

				double x_min = catalog->catalog->xmin[i];
				double y_min = catalog->catalog->ymin[i];
				double x_max = catalog->catalog->xmax[i];
				double y_max = catalog->catalog->ymax[i];
				...
				double theta = catalog->catalog->theta[i];

				double cxx = catalog->catalog->cxx[i];
				double cyy = catalog->catalog->cyy[i];
				double cxy = catalog->catalog->cxy[i];
				...
				{
					// Fit Moffat PSF
					if (!FitPSF(image, fit.attributes().data.out_dim.nx, catalog->catalog->x[i], catalog->catalog->y[i], x_min, y_min, x_max, y_max, &obj.psf, status))
					{
						continue;
					}
				}
