...
#define strcpy_s(dest, max_size, source) memcpy(dest, source, max_size)
#endif
...
extern "C"
{
	EXPORT FITSHandle LoadFit(const char* file_cstr, uint64_t max_input_size, uint32_t max_input_width, uint32_t max_input_height)
	{
	...
	EXPORT void CloseFitFile(FITSHandle handle)
	{
	...
	EXPORT bool ReadHeaderRecord(FITSHandle handle, int index, char* keyword, size_t nkeyword, char* value, size_t nvalue, char* comment, size_t ncomment)
	{
	...
	EXPORT void FreeFit(FITSHandle handle)
	{
		if (handle.info)
		{
			delete handle.info;
	...
	EXPORT bool ReadImage(FITSHandle handle, unsigned char* data, Loader::FITSImageLoaderParameters props)
	{
	...
	EXPORT FITSImageDataHandle LoadImageData(FITSHandle fits_handle, Loader::FITSImageLoaderParameters props, uint32_t* histogram, size_t histogram_size)
	{
	...
	EXPORT bool UnloadImageData(FITSImageDataHandle handle)
	{
	...
	EXPORT bool IsImageDataLoaded(FITSImageDataHandle handle)
	{
		return handle.image_ptr && *handle.image_ptr;
	}

	EXPORT void FreeImageData(FITSImageDataHandle handle)
	{
		if (handle.image_ptr)
		{
			if (*handle.image_ptr)
	...
	EXPORT FITSStatisticsHandle ComputeStatistics(FITSHandle fits_handle, FITSImageDataHandle data_handle, Photometry::Callback callback)
	{
	...
	EXPORT bool GetPhotometry(FITSStatisticsHandle handle, int src_start, int src_n, int dst_start, Photometry::Object* photometry)
	{
		if (!handle.catalog)
		{
			return false;
	...
	EXPORT void FreeStatistics(FITSStatisticsHandle handle)
	{
		if (handle.catalog)
		{
	...
	EXPORT Processing::ImageStretchParameters ComputeStretch(FITSHandle fits_handle, FITSImageDataHandle data_handle)
	{
	...
	EXPORT FITSImageHandle ProcessImage(FITSHandle fits_handle, FITSImageDataHandle data_handle, FITSImageHandle image_handle, bool compute_stretch_params, Loader::FITSImageLoaderParameters props, uint32_t* histogram, size_t histogram_size)
	{
		if (!fits_handle.info)
		{
			return image_handle;
	...
	EXPORT void FreeImage(FITSImageHandle handle)
	{
		if (handle.data_ptr)
		{
