	...
	void FITSInfo::ReadHeader()
	{
		...
		for (int hdu_num = 1; hdu_num <= nhdu; hdu_num++)
		{
			...
			if (hdu_type == IMAGE_HDU)
			{
				...
				std::transform(m_attributes.instrument.bayer_pattern.begin(), m_attributes.instrument.bayer_pattern.end(), m_attributes.instrument.bayer_pattern.begin(), to_lower);
				...
				std::transform(m_attributes.shot.filter.begin(), m_attributes.shot.filter.end(), m_attributes.shot.filter.begin(), to_lower);
